...
class PaymentRepository:
    ...
    def search_records(
        ...
        including Gold Layer trust score and risk level criteria.
        """
        db_sess = self.db
        if not db_sess:
            return [], 0
        ...
        # Optional join with GoldPlatformAnalytics if trust score / risk level filter is specified
        if min_trust_score is not None or max_trust_score is not None or risk_level:
            query = query.outerjoin(GoldPlatformAnalytics, func.lower(PaymentRecordModel.site) == func.lower(GoldPlatformAnalytics.site))
