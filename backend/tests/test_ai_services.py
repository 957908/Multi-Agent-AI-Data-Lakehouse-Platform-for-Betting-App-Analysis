...
Purpose:
    Unit tests for AI Intelligence services (Trust Engine, RAG Service, AI Analysis).
Author: Arjun Mehta
...
class TestTrustEngine(unittest.TestCase):
    ...
    def test_trust_score_calculation(self):
        """Test calculation of trust score with sample records."""
        resp = self.engine.calculate_platform_trust("melbet", custom_records=self.sample_records, sync_gold=False)
    ...
    def test_trust_score_empty_fallback(self):
        ...
        resp = self.engine.calculate_platform_trust("unknown_site", custom_records=[], sync_gold=False)
