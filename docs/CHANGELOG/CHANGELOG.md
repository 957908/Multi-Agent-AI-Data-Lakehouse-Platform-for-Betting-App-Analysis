...
### Added
- Standardized Playwright Site Adapters for Melbet, OneXBet, Tencric, and TwentyTwoBet.
- Configurable navigation timeouts, automatic retries with backoff delay, and connection diagnostics.
- Cookie expiration verification audits (`verify_session_cookies`), session cleanups, and auto-recovery.
- Strict mode selector locator protections via `.first` matches.
- Automated Data Lake raw evidence reorganizer (`data/raw/YYYY-MM-DD/HH-MM/`).
